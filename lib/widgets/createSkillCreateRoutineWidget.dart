...
import 'package:peaky_blinders/Pages/SkillSelectionCreateRoutineListPage.dart';
import 'package:peaky_blinders/widgets/TrapeziumLeftClipper.dart';

Widget createSelectedSkillssCreateRoutine(context, skills) {
  return Stack(
    children: <Widget>[
      Container(
...
Future navigateToSkillListPage(context) async {
  ...
  RoutineSettingBloc routineSettingBloc =
      BlocProvider.of<RoutineSettingBloc>(context);
  if (routineSettingBloc.selectedSkills.length == 0) {
    routineSettingBloc.skills = await skillBloc.getUnselectedSkills();
  } else {
    routineSettingBloc.skills = await skillBloc
        .setAllSkillByNewRoutineSetting(routineSettingBloc.selectedSkills);
  }

  await Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => SkillSelectionCreateRoutineListPage()));
}
